...
import { getChatReply, type ChatReply } from '../services/chatService'
...
export function FloatingTools() {
  ...
  const [messages, setMessages] = useState<ChatReply[]>([{ text: 'Hola, soy Lume. Puedo ayudarte a encontrar algo que se sienta muy tú.' }])
  const [input, setInput] = useState('')
  const send = async (message = input) => {
    if (!message.trim()) return
    setMessages((current) => [...current, { text: message }])
    setInput('')
